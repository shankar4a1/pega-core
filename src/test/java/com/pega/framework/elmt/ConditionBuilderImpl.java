...
public class ConditionBuilderImpl extends PegaWebElementImpl implements ConditionBuilder
{
    ...
    public void bootstrapRow() {
        ...
        this.anyPicker = this.findAnyPicker(String.valueOf(this.curRowXPath) + "//div[@data-test-id='20160509045644070512696']");
    ...
    public void addRow(final String recordType, int rowToAppend) {
        this.findElement(By.xpath(String.valueOf(this.curRowXPath) + "//div[@data-test-id='201803161342510972763']" + "//a")).click();
        this.pegaDriver.switchToActiveFrame();
        this.curRowIndex = ++rowToAppend;
        this.curRowType = this.getRecordTypeEnum(recordType);
        this.bootstrapRow();
        this.curRowIdentifier = this.findElement(By.xpath(String.valueOf(this.curRowXPath) + "//span[@data-test-id='2018110409142809601240']")).getText();
        this.setLogicString(this.curAdvancedLogicString = String.valueOf(this.curAdvancedLogicString) + " AND " + this.curRowIdentifier);
    }
    ...
    public void setLogicStyle(final String logicStyle) {
        ...
        final List<WebElement> advancedMode = (List<WebElement>)this.pegaDriver.findElements(By.xpath("//button[@data-test-id='201608041031140988489']"));
    ...
    private void setOperator(final String operator) {
        this.curRow.findSelectBox(By.xpath(String.valueOf(this.curRowXPath) + "//select[@data-test-id='2016030403503300729539']")).selectByVisibleText(operator);
        this.pegaDriver.switchToActiveFrame();
    }
    
    private void setRightField(final String value) {
        if (this.curRowType.getType() == "Strategy") {
            this.curRow.findSelectBox(By.xpath(String.valueOf(this.curRowXPath) + "//select[@data-test-id='20151012085636097134858']")).selectByVisibleText(value);
        }
        else {
            this.curRow.findElement(By.xpath(String.valueOf(this.curRowXPath) + "//input[@data-test-id='20151012085636097134858']")).sendKeys(value);
        }
        this.pegaDriver.switchToActiveFrame();
    }
    
    private void setBooleanField(final boolean value) {
        this.curRow.findSelectBox(By.xpath(String.valueOf(this.curRowXPath) + "//select[@data-test-id='2016030403503300729539']")).selectByVisibleText("is " + Boolean.toString(value));
    }
