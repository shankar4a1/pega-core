...
public class FrameImpl implements Frame
{
    ...
    public void ReturnElementsFromApp(final By by, final WebElement element) {
        ...
        if (by.toString() != null && !by.toString().contains("data-test-id")) {
            ...
            if (element.getAttribute("data-test-id") == null) {
                FrameImpl.uniqueReturnElements.add(String.valueOf(searchElement) + "~'" + element.getAttribute("data-test-id") + "'~0~" + element.getTagName() + "~N");
            }
            else if (this.findElements(By.xpath("//" + element.getTagName() + "[@data-test-id = '" + element.getAttribute("data-test-id") + "']")).size() > 1) {
                FrameImpl.uniqueReturnElements.add(String.valueOf(searchElement) + "~'" + element.getAttribute("data-test-id") + "'~" + this.findElements(By.xpath("//" + element.getTagName() + "[@data-test-id = '" + element.getAttribute("data-test-id") + "']")).size() + "~" + element.getTagName() + "~N");
            }
            else {
                FrameImpl.uniqueReturnElements.add(String.valueOf(searchElement) + "~'" + element.getAttribute("data-test-id") + "'~" + this.findElements(By.xpath("//" + element.getTagName() + "[@data-test-id = '" + element.getAttribute("data-test-id") + "']")).size() + "~" + element.getTagName() + "~N");
            }
            ...
            FrameImpl.uniqueReturnElements.add(String.valueOf(searchElement) + "~'dataTestIdUsed'~datatestIdused~" + element.getTagName() + "~Y");
        }
    }
    ...
    public Frame findFrame(final String frameId) {
        ...
        frame._setDOMPointer(String.valueOf(this.getFrameDocument()) + ".defaultView.frames['" + frameId + "']");
    ...
    public ConditionBuilder findConditionBuilder(final By by) {
        ...
        this.switchToCurrentFrame();
        this.testEnv.getPegaDriver().handleWaits().waitForElementPresence(by);
        final List<WebElement> elmts = (List<WebElement>)this.driver.findElements(by);
        if (elmts.size() == 1) {
    ...
    public String getFrameDocument() {
        if (this.frameDoc == null) {
            if (this.getDOMPointer() != null) {
                ...
                this.frameDoc = String.valueOf(this.getDOMPointer()) + document;
            }
            else {
    ...
    public List<WebElement> findElements(final By by) {
        this.pegaDriver.handleWaits().verifyAndWaitIfThrobberPresent();
        this.switchToCurrentFrame();
        return (List<WebElement>)this.pegaDriver.getDriver().findElements(by);
    }
    ...
    public Frame findFrameByName(final String frameName) {
        ...
        frame._setDOMPointer(String.valueOf(this.getFrameDocument()) + ".defaultView.frames['" + frameName + "']");
        FrameImpl.LOGGER.debug("driver location before frame switch:" + this.driver.getCurrentUrl());
