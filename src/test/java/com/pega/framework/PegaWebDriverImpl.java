...
import com.pega.framework.elmt.AutoCompleteImpl;
import com.pega.ri.WizardImpl;
import com.pega.ri.Wizard;
import com.pega.framework.elmt.DropDownImpl;
...
import com.pega.util.ElementUtil;
...
public class PegaWebDriverImpl implements PegaWebDriver
{
    ...
    protected String generateDOMPath(final String document, final By by) {
        ...
        if (byLoc != null && byLoc.contains(":")) {
            ...
            if (type.contains("id")) {
                domPointer = String.valueOf(document) + ".getElementById('" + locator + "')";
            }
            else if (type.contains("name")) {
                domPointer = String.valueOf(document) + ".getElementsByName('" + locator + "')[0]";
            }
            else if (type.contains("selector")) {
                domPointer = String.valueOf(document) + ".querySelector('" + locator + "')";
            }
            else if (type.contains("xpath")) {
                domPointer = String.valueOf(document) + ".evaluate(\"" + locator + "\"," + document + ".body, null, XPathResult.ANY_TYPE, null).iterateNext()";
            }
            else if (type.contains("linkText")) {
                domPointer = String.valueOf(document) + ".evaluate(\"//a[text()='" + locator + "']\"," + document + ".body, null, XPathResult.ANY_TYPE, null).iterateNext()";
            }
            else if (type.contains("partialLinkText")) {
                domPointer = String.valueOf(document) + ".evaluate(\"//a[contains(text(), '" + locator + "')]\"," + document + ".body, null, XPathResult.ANY_TYPE, null).iterateNext()";
            }
            else if (type.contains("className")) {
                domPointer = String.valueOf(document) + ".getElementsByClassName('" + locator + "')[0]";
            }
            else if (type.contains("tagName")) {
                domPointer = String.valueOf(document) + ".getElementsByTagName('" + locator + "')[0]";
            }
        }
        return domPointer;
    }
    ...
    public List<WebElement> findElements(final By by) {
        ...
        return (List<WebElement>)this.driver.findElements(by);
    }
    ...
    public Set<String> getWindowHandles() {
        return (Set<String>)this.driver.getWindowHandles();
    }
    ...
    public String getActiveFrameId(final boolean switchToActiveFrame) {
        ...
        if (isLoaded != null) {
            ...
            while (!status && counter < timeOut) {
                ...
                if (counter % 60 == 0) {
                    PegaWebDriverImpl.LOGGER.debug(String.valueOf((timeOut - counter) / 60) + " more minutes to go");
                }
            }
        }
    ...
    public PegaWebElement convertToPegaWebElmt(final WebElement elmt, final By by, final Frame frame) {
        final PegaWebElement pegaWebElmt = new PegaWebElementImpl(elmt);
        final String document = (frame == null) ? "document" : (String.valueOf(frame.getDOMPointer()) + ".ownerDocument");
        pegaWebElmt._setEnvironment(this.testEnv, by, document);
        PegaWebDriverImpl.LOGGER.debug("DOM Pointer for element identified by: \"" + by + "\" is : " + pegaWebElmt.getDOMPointer());
        return pegaWebElmt;
    }
    ...
    public AutoComplete findAutoComplete(final By by) {
        ...
        PegaWebDriverImpl.LOGGER.debug("DOM Pointer for element identified by: \"" + by + "\" is : " + autoComplete.getDOMPointer(), (Object)true);
        return autoComplete;
    }
    ...
    public int getDefaultFrameTabCntDiff(final boolean login) {
        if (login) {
            PegaWebDriverImpl.LOGGER.debug("Initializing default tab and frame count difference after login ", (Object)true);
            ...
            PegaWebDriverImpl.LOGGER.debug("Tab frame count difference is : " + this.tabFrameCntDiff, (Object)true);
        }
    ...
    public void ReturnElementsFromApp(final By by, final WebElement element) {
        ...
        if (by2.length < 3) {
            for (int i = 1; i < by2.length; ++i) {
                try {
                    instring = String.valueOf(instring) + by2[i];
                }
        ...
        searchElement = String.valueOf(by2[0]) + "(\"" + instring.trim() + "\")";
        ...
        if (by.toString() != null && !by.toString().contains("data-test-id")) {
            ...
            if (element.getAttribute("data-test-id") == null) {
                PegaWebDriverImpl.uniqueReturnElements.add(String.valueOf(searchElement) + "~'" + element.getAttribute("data-test-id") + "'~0~" + element.getTagName() + "~N");
            }
            else if (this.findElements(By.xpath("//" + element.getTagName() + "[@data-test-id = '" + element.getAttribute("data-test-id") + "']")).size() > 1) {
                PegaWebDriverImpl.uniqueReturnElements.add(String.valueOf(searchElement) + "~'" + element.getAttribute("data-test-id") + "'~" + this.findElements(By.xpath("//" + element.getTagName() + "[@data-test-id = '" + element.getAttribute("data-test-id") + "']")).size() + "~" + element.getTagName() + "~N");
            }
            else {
                PegaWebDriverImpl.uniqueReturnElements.add(String.valueOf(searchElement) + "~'" + element.getAttribute("data-test-id") + "'~" + this.findElements(By.xpath("//" + element.getTagName() + "[@data-test-id = '" + element.getAttribute("data-test-id") + "']")).size() + "~" + element.getTagName() + "~N");
            }
            ...
            PegaWebDriverImpl.uniqueReturnElements.add(String.valueOf(searchElement) + "~'dataTestIdUsed'~datatestIdused~" + element.getTagName() + "~Y");
        }
    }
