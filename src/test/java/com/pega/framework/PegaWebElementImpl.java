...
import com.pega.config.test.ThreadLocalStepDefinitionMatch;
...
import com.pega.ri.InstanceImpl;
...
public class PegaWebElementImpl extends InstanceImpl implements PegaWebElement
{
    ...
    protected String generateDOMPath(final String document, final By by) {
        ...
        if (byLoc != null && byLoc.contains(":")) {
            ...
            if (type.contains("id")) {
                domPinter = String.valueOf(document) + ".getElementById('" + locator + "')";
            }
            else if (type.contains("name")) {
                domPinter = String.valueOf(document) + ".getElementsByName('" + locator + "')[0]";
            }
            else if (type.toLowerCase().contains("selector")) {
                locator = locator.replace("'", "\\'");
                domPinter = String.valueOf(document) + ".querySelector('" + locator + "')";
            }
            else if (type.contains("xpath")) {
                domPinter = String.valueOf(document) + ".evaluate(\"" + locator + "\"," + document + ".body, null, XPathResult.ANY_TYPE, null).iterateNext()";
            }
            else if (type.contains("linkText")) {
                domPinter = String.valueOf(document) + ".evaluate(\"//a[text()='" + locator + "']\"," + document + ".body, null, XPathResult.ANY_TYPE, null).iterateNext()";
            }
            else if (type.contains("partialLinkText")) {
                domPinter = String.valueOf(document) + ".evaluate(\"//a[contains(text(), '" + locator + "')]\"," + document + ".body, null, XPathResult.ANY_TYPE, null).iterateNext()";
            }
            else if (type.contains("className")) {
                domPinter = String.valueOf(document) + ".getElementsByClassName('" + locator + "')[0]";
            }
            else if (type.contains("tagName")) {
                domPinter = String.valueOf(document) + ".getElementsByTagName('" + locator + "')[0]";
            }
        }
        return domPinter;
    }
    ...
    public void scrollIntoView() {
        if (this.testEnv.getConfiguration().getPlatformDetails().isLinux()) {
            ...
            ((JavascriptExecutor)this.pegaDriver.getDriver()).executeScript("arguments[0].scrollIntoView();", new Object[] { this.getWebElement() });
        }
    }
    ...
    public int getElementLeft() {
        ...
        final long left = (long)this.scriptExecutor.executeJavaScript(script);
        return (int)left;
    }
    ...
    public List<WebElement> findElements(final By by) {
        ...
        return (List<WebElement>)this.elmt.findElements(by);
    }
    ...
    public DropDown findSelectBox(final By by) {
        ...
        final WebElement elmt = (WebElement)this.findElement(by);
        if (this.testEnv.getConfiguration().analyseDataTestId()) {
    ...
    public void ReturnElementsFromApp(final By by, final WebElement element) {
        ...
        if (by.toString() != null && !by.toString().contains("data-test-id")) {
            ...
            if (element.getAttribute("data-test-id") == null) {
                PegaWebElementImpl.uniqueReturnElements.add(String.valueOf(searchElement) + "~'" + element.getAttribute("data-test-id") + "'~0~" + element.getTagName() + "~N");
            }
            else if (this.findElements(By.xpath("//" + element.getTagName() + "[@data-test-id = '" + element.getAttribute("data-test-id") + "']")).size() > 1) {
                PegaWebElementImpl.uniqueReturnElements.add(String.valueOf(searchElement) + "~'" + element.getAttribute("data-test-id") + "'~" + this.findElements(By.xpath("//" + element.getTagName() + "[@data-test-id = '" + element.getAttribute("data-test-id") + "']")).size() + "~" + element.getTagName() + "~N");
            }
            else {
                PegaWebElementImpl.uniqueReturnElements.add(String.valueOf(searchElement) + "~'" + element.getAttribute("data-test-id") + "'~" + this.findElements(By.xpath("//" + element.getTagName() + "[@data-test-id = '" + element.getAttribute("data-test-id") + "']")).size() + "~" + element.getTagName() + "~N");
            }
            ...
            PegaWebElementImpl.uniqueReturnElements.add(String.valueOf(searchElement) + "~'dataTestIdUsed'~datatestIdused~" + element.getTagName() + "~Y");
        }
    }
