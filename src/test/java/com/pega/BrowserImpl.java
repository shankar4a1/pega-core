...
public class BrowserImpl implements Browser
{
    ...
    public void logout() {
        ...
        this.pegaDriver.get(String.valueOf(this.testEnv.getConfiguration().getSUTConfig().getURL()) + "/!Developer?pyActivity=Code-Security.EndSession");
        this.pegaDriver.handleWaits().waitForPageLoaded();
    }
    ...
    public String getWindow(final int windowNo) {
        final Set<String> set = (Set<String>)this.driver.getWindowHandles();
    ...
    public boolean switchToWindow(final int windowNo, final boolean waitForPageLoaded) {
        ...
        BrowserImpl.LOGGER.error("Switching could not be done to window no: " + windowNo, (Object)true);
        return false;
    }
    ...
    public boolean switchToWindow(final String title) {
        ...
        final Set<String> set = (Set<String>)this.pegaDriver.getWindowHandles();
        final Iterator<String> iter = set.iterator();
        while (iter.hasNext()) {
            this.pegaDriver.switchTo().window((String)iter.next());
            this.pegaDriver.handleWaits().waitForPageLoaded();
            this.pegaDriver.handleWaits().sleep(1L);
    ...
    private void enableFullScreenForIE() {
        try {
            final Wait<WebDriver> wait = (Wait<WebDriver>)new WebDriverWait(this.driver, 30L);
            wait.until((Function)this.checkFullScreenStatus());
        }
        catch (TimeoutException te) {
            BrowserImpl.LOGGER.error("Could not do a full screen mode after timeout. Continuing with available resolution", (Object)true);
        }
    }
            ...
            public Boolean apply(final WebDriver driver) {
                try {
                    ...
                    if (screenHeight - browserHeight > 30L) {
                        ...
                        BrowserImpl.this.pegaDriver.findElement(By.tagName("body")).sendKeys((CharSequence)Keys.F11);
                        BrowserImpl.this.pegaDriver.handleWaits().sleep(2L);
    ...
    public String getDownloadDir() {
        ...
        if (this.testEnv.getConfiguration().getPlatformDetails().isWindows()) {
            downloads_path = String.valueOf(System.getProperty("user.home")) + "/Downloads/";
        }
        else {
            downloads_path = String.valueOf(System.getenv("HOME")) + "/Downloads/";
        }
