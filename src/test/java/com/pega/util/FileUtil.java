...
public class FileUtil
{
    ...
    public static void writeToFileInUTF8(final String filePath, final String textToWrite) throws IOException {
        ...
        try {
            Throwable t = null;
            try {
                final BufferedWriter out = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(fileDir), "UTF8"));
                try {
                    out.append((CharSequence)textToWrite);
                }
                finally {
                    if (out != null) {
                        out.close();
    ...
    public static void copyFile(final String source, final String destination) {
        try {
            final Path dest = Paths.get(destination, new String[0]);
            final Path src = Paths.get(source, new String[0]);
    ...
    public static void zip(final String targetFolderOrFileToZip_Path, final String zipFilePath, final boolean includeTopLevelDirectory, final String password) throws IOException {
        try {
            ...
            if (zip.exists()) {
                Files.delete(FileSystems.getDefault().getPath(zipFilePath, new String[0]));
            }
    ...
    public static int getCSVRowCount(final String fileName) throws IOException {
        Scanner scanIn = null;
        int numberOfRows = 0;
        final String file = String.valueOf(System.getProperty("user.dir")) + File.separator + DataUtil.getDataFolder() + File.separator + fileName;
    ...
    public static void setCSVData(final String fileName, final int rowNumber, final int columnNumber, final String newValue, final String delimiter) throws IOException {
        ...
        final File file = new File(String.valueOf(System.getProperty("user.dir")) + File.separator + DataUtil.getDataFolder() + File.separator + fileName);
        final List<String> lines = (List<String>)FileUtils.readLines(file);
        String[] arrayvalues = new String[0];
        if (lines != null && lines.get(rowNumber) != null && lines.get(rowNumber).contains(String.valueOf(output) + ",")) {
        ...
        lines.add(rowNumber, StringUtils.join((Object[])arrayvalues, ","));
            ...
            try {
                ...
                try {
                    ...
                    for (final String csvData : header) {
                        csvRow = String.valueOf(csvRow) + csvData + COMMA_DELIMITER;
                    }
                    ...
                    for (final List<String> list : data) {
                        ...
                        for (final String csvData2 : list) {
                            csvRow = String.valueOf(csvRow) + csvData2 + COMMA_DELIMITER;
                        }
                        csvRow = csvRow.substring(0, csvRow.length() - 1);
                        fileWriter.append(csvRow);
                        fileWriter.append(NEW_LINE_SEPARATOR);
    ...
    public static void copySheet(final String srcFilePath, final String destFilePath, final String sheetName) {
        ...
        try {
            Throwable t = null;
            try {
                final XSSFWorkbook newWB = new XSSFWorkbook((InputStream)new FileInputStream(new File(destFilePath)));
                try {
                    oldWB = new XSSFWorkbook((InputStream)new FileInputStream(new File(srcFilePath)));
                    ...
                    for (int rowIndex = 0; rowIndex < sheetFromOldWB.getPhysicalNumberOfRows(); ++rowIndex) {
                        ...
                        for (int colIndex = 0; colIndex < sheetFromOldWB.getRow(rowIndex).getPhysicalNumberOfCells(); ++colIndex) {
                            ...
                            if (c.getCellType() != 3) {
                                ...
                                newStyle.cloneStyleFrom((CellStyle)origStyle);
                    ...
                    newWB.write((OutputStream)fileOut);
                    fileOut.close();
                }
                finally {
                    if (newWB != null) {
    ...
    public static List<List<String>> readSheet(final String excelFilePath, final int sheetIndex, final int noOfHeaderRowsToIgnore, final boolean ignoreEmptyCells) {
        ...
        try {
            wb = new XSSFWorkbook((InputStream)new FileInputStream(new File(excelFilePath)));
    ...
    public static void deleteSheet(final String excelFilePath, final int sheetIndex) throws IOException {
        ...
        try {
            book = new XSSFWorkbook((InputStream)new FileInputStream(excelFile));
            book.removeSheetAt(sheetIndex);
            book.write((OutputStream)new FileOutputStream(excelFile));
        }
        catch (FileNotFoundException e) {
            e.printStackTrace();
        }
