...
import com.pega.exceptions.NonLocalizedWordsException;
...
public class LocalizationUtil
{
        ...
        while (repeat) {
            ...
            if (word.contains("({.py") || word.contains("{.py")) {
                ...
                while (matcher.find()) {
                    ...
                    pyText = String.valueOf(pyText) + match + " ";
                ...
                if (word.startsWith("py") || (word.startsWith(".py") && !word.contains(" "))) {
                    ...
                    while (matcher2.find()) {
                        ...
                        pyText2 = String.valueOf(pyText2) + match2 + " ";
            ...
            String currentWord = StringUtils.join((Object[])StringUtils.splitByCharacterTypeCamelCase(wordToConvert), ' ').trim();
            String newUrl = String.valueOf(url) + URLEncoder.encode(currentWord.replaceAll(" +", " ").replaceAll("\\.", ":").replaceAll("& nbsp ;", ":").replaceAll(";", ":").replaceAll("&", ":").replaceAll("\\\t", "").replaceAll("\\\n", "").replaceAll("#", ":").replaceAll("\\|", ":").replaceAll(";", ":").replaceAll("\\?", ":").replaceAll("!", ":"), "UTF-8");
            while (i < wordsToConvert.size()) {
                ...
                if (word.contains("({.py") || word.contains("{.py")) {
                    ...
                    while (matcher3.find()) {
                        ...
                        pyText3 = String.valueOf(pyText3) + match3 + " ";
                ...
                currentWord = StringUtils.join((Object[])StringUtils.splitByCharacterTypeCamelCase(wordToConvert), ' ').trim();
                newUrl = String.valueOf(newUrl) + "%0A" + URLEncoder.encode(currentWord.replaceAll(" +", " ").replaceAll("\\.", ":").replaceAll("& nbsp ;", ":").replaceAll(";", ":").replaceAll("&", ":").replaceAll("\\\t", "").replaceAll("\\\n", "").replaceAll("#", ":").replaceAll("\\|", ":").replaceAll(";", ":").replaceAll("\\?", ":").replaceAll("!", ":"), "UTF-8");
                Reporter.log("Current i position: " + i, true);
                if (i % 25 == 0) {
                    ++i;
                    break;
            ...
            try {
                ...
                while (matcher3.find()) {
                    ...
                    translatedTextList.add((String.valueOf(translatedText) + " " + pyExcludedWords.get(k++)).trim());
                }
            }
            catch (Exception e) {
                e.printStackTrace();
    ...
    private static Set<String> getEnglishDictionary() {
        ...
        Scanner file1 = null;
        try {
            file1 = new Scanner(new File(String.valueOf(System.getProperty("user.dir")) + "/Data/EnglishDictionary.txt"));
            while (file1.hasNext()) {
    ...
    private static List<String> getTranslatedWords() throws IOException {
        ...
        try {
            ...
            if (fileName.endsWith(".xml")) {
                final Reader reader = new InputStreamReader(new FileInputStream(fileName), "utf-8");
                ...
                try {
                    getTranslatedProperties().load(new InputStreamReader(new FileInputStream(new File(fileName)), "UTF-8"));
                }
                catch (Exception e) {
    ...
    private static void parse(String text) {
        ...
        if (nonLocalWords.size() > 0) {
            ...
            message = String.valueOf(message) + "+--------------------------------------------+\n";
            message = String.valueOf(message) + "End of non Localized words list\n";
        }
        if (!message.equals("")) {
    ...
    private static Map<String, String> getTranslationsMap() throws IOException {
        if (LocalizationUtil.translationsMap == null) {
            ...
            if (LocalizationUtil.languageBundlePath.endsWith(".properties")) {
                ...
                BufferedReader br = null;
                try {
                    final Reader reader = new InputStreamReader(new FileInputStream(getTranslationfilepath()), "utf-8");
                    br = new BufferedReader(reader);
                    int i = 1;
    ...
    private static String getTranslationfilepath() {
        if (!LocalizationUtil.languageBundlePath.endsWith(".properties")) {
            if (LocalizationUtil.xmlFilePath == null && !LocalizationUtil.language.equalsIgnoreCase(HTTPUtil.TransaltionLang.ENGLISH.getLang())) {
                ...
                LocalizationUtil.languageBundlePath = String.valueOf(System.getProperty("user.dir")) + System.getProperty("file.separator") + DataUtil.getDataFolder() + System.getProperty("file.separator") + LocalizationUtil.languageBundlePath;
    ...
    private static void initializeTranslatedProperties() {
        if (!LocalizationUtil.language.equalsIgnoreCase(HTTPUtil.TransaltionLang.ENGLISH.getLang())) {
            ...
            try {
                ...
                LocalizationUtil.translatedProperties.load(new InputStreamReader(new FileInputStream(new File(LocalizationUtil.languageBundlePath)), "UTF-8"));
            }
            catch (Exception e) {
                throw new RuntimeException("Problem while reading transalted properties file.", e);
            }
    ...
    public static void createPropertiesFileFromXML(final String xmlFilePath, final String propertiesFilePath) throws IOException {
        ...
        try {
            p2.store(new OutputStreamWriter(new FileOutputStream(propertiesFilePath), "UTF-8"), "translated bundle");
        }
        catch (IOException e2) {
            e2.printStackTrace();
        }
