...
public class ExcelUtil
{
    public static void writeToExcel(final String path, final String sheet_name, final int rowcount, final int colcount, final Object value) {
        ...
        try {
            ...
            if (!isSheetExist) {
                Reporter.log(String.valueOf(sheet_name) + ": sheet does not exist in the " + path + " excel file", true);
            }
                    ...
                    Label_0354: {
                        if (path.endsWith("xls")) {
                            try {
                                Throwable t2 = null;
                                try {
                                    final Workbook hssfWorkbook = (Workbook)new HSSFWorkbook((InputStream)fis);
                                    try {
                        ...
                        try {
                            Throwable t3 = null;
                            try {
                                final Workbook xssfWorkbook = (Workbook)new XSSFWorkbook((InputStream)fis);
                                try {
    ...
    public static Date readDateFromExcel(final SimpleDateFormat sdf, final String path, final String sheetName, final int rowIndex, final int cellIndex) {
        ...
        Label_0282: {
            try {
                ...
                if (!isSheetExisting) {
                    Reporter.log(String.valueOf(sheetName) + ": sheet does not exist in the " + path + " excel file", true);
                }
