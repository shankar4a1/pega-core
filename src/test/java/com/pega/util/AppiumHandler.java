...
import com.pega.exceptions.NotYetImplementedException;
...
public class AppiumHandler
{
    ...
    static {
        LOGGER = LoggerFactory.getLogger((Class)AppiumHandler.class);
    }
    ...
    public void startAppium(final String appiumNodePath, final String appiumJsPath, final String appiumLogsPath, final String appiumServerAdress, final String appiumPort) throws IOException, InterruptedException {
        ...
        if (this.config.getPlatformDetails().isWindows()) {
            ...
            final PumpStreamHandler streamHandler = new PumpStreamHandler((OutputStream)outputStream);
    ...
    private boolean isProcessRunning(final String process) throws IOException {
        ...
        if (this.config.getPlatformDetails().isWindows()) {
            ...
            while ((line = input.readLine()) != null) {
                pidInfo = String.valueOf(pidInfo) + line;
            }
            input.close();
            if (pidInfo.contains(process)) {
                AppiumHandler.LOGGER.debug(String.valueOf(process) + " Server running");
                return true;
            }
            AppiumHandler.LOGGER.debug(String.valueOf(process) + "Server not running");
            return false;
        }
        else {
            if (!this.config.getPlatformDetails().isMac() || !"ios".equalsIgnoreCase(this.config.getPlatformDetails().getPlatform())) {
            ...
            if (i > 1) {
                AppiumHandler.LOGGER.debug(String.valueOf(i) + " " + process + " processes are running");
                return true;
            }
            AppiumHandler.LOGGER.debug(String.valueOf(process) + " Server not running");
            return false;
