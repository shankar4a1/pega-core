...
public class TestBase extends TestEnvironmentImpl
{
    ...
    private void captureScreenshot(final Scenario scenario) {
        if (scenario.isFailed()) {
            try {
                final byte[] screenshot = (byte[])((TakesScreenshot)this.getPegaDriver().getDriver()).getScreenshotAs(OutputType.BYTES);
                scenario.embed(screenshot, "image/png");
                final File temp = (File)((TakesScreenshot)this.getPegaDriver().getDriver()).getScreenshotAs(OutputType.FILE);
    ...
    protected void tearDown(final Scenario scenario, final boolean performLogout, final boolean saveVideoForPassedScenario) {
        ...
        this.captureScreenshot(scenario);
        if (!this.isDebugMode) {
        ...
        if (this.getConfiguration().getSUTConfig().isEnableVideoRecording()) {
            ...
            if (jobTags != null) {
                if (!reportsDir.contains(jobTags)) {
                    reportsDir = String.valueOf(reportsDir) + System.getProperty("file.separator") + jobTags;
                }
            }
            else {
                ...
                if (feature != null) {
                    ...
                    reportsDir = String.valueOf(reportsDir) + System.getProperty("file.separator") + feature;
                }
            }
            ...
            try {
                ...
                this.videoFilePath = String.valueOf(reportsDir) + System.getProperty("file.separator") + videoFileName + ".avi";
    ...
    private void deleteVideoFile() {
        if (System.getenv("JENKINS_URL") != null) {
            try {
                ...
                TestBase.LOGGER.error("Unable to delete video file: " + this.videoFilePath, (Object)40000);
            }
        }
    }
                ...
                if (scenario.isFailed()) {
                    try {
                        ...
                        if (System.getenv("job.name") != null) {
                            ...
                            url = String.valueOf(System.getenv("JENKINS_URL")) + "job/" + System.getenv("job.name") + "/ws/" + "ArchivedReports" + "/%23" + buildId + "- " + System.getenv("team.name") + "/" + tags + "/" + tcTags + ".avi";
                        }
                        else {
                            url = String.valueOf(System.getenv("JOB_URL")) + "ws/" + "ArchivedReports" + "/%23" + System.getenv("BUILD_NUMBER") + "- " + System.getenv("team.name") + "/" + tags + "/" + tcTags + ".avi";
                        }
                        url.replace(" ", "%20");
