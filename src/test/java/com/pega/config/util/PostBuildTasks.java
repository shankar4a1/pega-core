...
public class PostBuildTasks
{
    ...
    public static void main(final String[] args) throws FileNotFoundException, IOException, URISyntaxException, ParserConfigurationException, SAXException {
    ...
    private void cleanup() {
        final File buildDir = new File(String.valueOf(System.getProperty("user.dir")) + File.separator + System.getenv("JOB_NAME") + "_" + System.getenv("BUILD_NUMBER"));
        ...
        final File logFile = new File(String.valueOf(System.getProperty("user.dir")) + File.separator + "log-" + System.getenv("BUILD_NUMBER") + ".txt");
    ...
    private void initVars() throws FileNotFoundException, IOException, ParserConfigurationException, SAXException {
        ...
        else if (teamJobRun.trim().equalsIgnoreCase("true")) {
            this.isTeamJobExecution = true;
        }
        else {
            this.isTeamJobExecution = false;
        }
    ...
    private void getTestStatistics() throws ParserConfigurationException, SAXException, IOException {
        try {
            final String url = String.valueOf(this.buildURL) + "testReport/api/xml?pretty=true";
    ...
    private void sendRequests() throws IOException, URISyntaxException {
        if (this.isFAIgnored) {
            ...
            this.failureAnalysisURL = (this.failureAnalysisURL.endsWith("/") ? this.failureAnalysisURL : (String.valueOf(this.failureAnalysisURL) + "/"));
            this.analyzeLinkURL = String.valueOf(this.failureAnalysisURL) + "postval?job=" + this.jobName.replace(" ", "%20") + "&testRun=" + this.buildId;
            String getAllTestsURL = String.valueOf(this.failureAnalysisURL) + "getAll?job=" + this.jobName.replace(" ", "%20") + "&testRun=" + this.buildId;
            final String backUpURL = String.valueOf(this.failureAnalysisURL) + "takeBackUp";
            if (this.isTeamJobExecution) {
                ...
                this.analyzeLinkURL = String.valueOf(this.analyzeLinkURL) + "&teamName=" + this.teamName + scenarioNameParam;
                getAllTestsURL = String.valueOf(getAllTestsURL) + "&teamName=" + this.teamName + scenarioNameParam;
            }
    ...
    private void sendEmail() {
        ...
        if (this.fromPassword != null) {
            ...
            session = Session.getInstance(properties, (Authenticator)new Authenticator() {
        ...
        try {
            ...
            message.setFrom((Address)new InternetAddress(this.fromEmailAddress));
            if (!recepientsList.contains("MissionImpossible@pega.com") && (this.isTeamJobExecution || (!this.isMIExcluded && !this.isDevJenkins))) {
                message.addRecipient(Message.RecipientType.CC, (Address)new InternetAddress("MissionImpossible@pega.com"));
            }
            if (this.ccList != null) {
                final String[] ids = this.ccList.split(",");
                for (int i = 0; i < ids.length; ++i) {
                    message.addRecipient(Message.RecipientType.CC, (Address)new InternetAddress(ids[i]));
                }
            }
            final Iterator<String> iter = recepientsList.iterator();
            while (iter.hasNext()) {
                message.addRecipient(Message.RecipientType.TO, (Address)new InternetAddress((String)iter.next()));
            }
            if (this.customMessage != null) {
                message.setSubject(String.valueOf(this.customMessage) + "- Tests Execution Results");
            }
            ...
            message.setContent((Object)this.getMailBody(), "text/html; charset=UTF-8");
    ...
    private String getMailBody() {
        ...
        if (this.excludePrpcDetails == null) {
            ...
            envDetails = String.valueOf(envDetails) + "<tr><th>PRPC Build</th><td>" + this.prpcBuild + "</td></tr>" + "<tr><th>PRPC Version</th><td>" + "Universal SMA v7.4" + "</td></tr>";
        }
        envDetails = String.valueOf(envDetails) + "<tr><th>Tags</th><td>" + this.tags + "</td></tr>" + "<tr><th>Browser</th><td>" + this.browser + "</td></tr>" + "</table>";
        double passPer;
        double failPer;
        double skipPer;
        ...
        String text = String.valueOf(header) + "<body><div><p>";
        text = String.valueOf(text) + chart;
        text = String.valueOf(text) + "Hi, <br><br>Please find your execution details below:<br><br>" + jobDetails + envDetails + "<b>For Cucumber Reports:</b>" + "<a href=\"" + this.buildURL + "cucumber-html-reports/feature-overview.html\">Click here</a>" + "<br/><br/><b>For Build log:</b><a href=\"" + this.buildURL + "console\">Click Here</a>";
        if (!this.isFAIgnored) {
            text = String.valueOf(text) + "<br/><br/><b>For Failure Analysis:</b><a href=\"" + this.analyzeLinkURL + "\">Click Here</a>";
        }
        text = String.valueOf(text) + "<br/><br/><br>Thanks,<br>";
        if (this.customSignature == null) {
            text = String.valueOf(text) + "Mission Impossible";
        }
        else {
            text = String.valueOf(text) + this.customSignature;
        }
        text = String.valueOf(text) + "</p></div></body></html>";
        return text;
    }
    ...
    public enum Status
    {
        ...
        private Status(final String name, final int ordinal, final String value) {
            this.value = value;
        }
