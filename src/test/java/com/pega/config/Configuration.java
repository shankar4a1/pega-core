...
import java.net.URL;
import com.pega.config.util.FetchPRPCDetails;
...
public class Configuration
{
        ...
        if (tags.contains(",")) {
            ...
            for (int i = 0; i < multipleTags.length; ++i) {
                if (multipleTags[i].startsWith("~")) {
                    excludeGroups = String.valueOf(multipleTags[i].trim()) + ",";
                }
                else {
                    includeGroups = String.valueOf(multipleTags[i].trim()) + ",";
                }
            }
        }
        else if (tags.startsWith("~")) {
            excludeGroups = String.valueOf(tags.trim()) + ",";
        }
        else {
            includeGroups = String.valueOf(tags.trim()) + ",";
        }
    ...
    private String updateInstanceURLForAutoTriggers() throws FileNotFoundException, IOException {
        String prpcUrl = null;
    ...
    private void downloadFile(final String fileURL, final String saveDir) throws IOException {
        ...
        Label_0262: {
            if (responseCode == 200) {
                ...
                final String saveFilePath = String.valueOf(saveDir) + File.separator + fileName;
                try {
    ...
    private String getPRPCVersion(final String prpcUrl) throws MalformedURLException, IOException {
    ...
    private void cleanUpDirs() throws IOException {
        final File f1 = new File(String.valueOf(System.getenv("WORKSPACE")) + System.getProperty("file.separator") + "LatestReports");
        final File f2 = new File(String.valueOf(System.getenv("WORKSPACE")) + System.getProperty("file.separator") + "AggregatedReports");
        if (f1.exists()) {
    ...
    private void resetTestExecutionStatus() throws IOException {
        ...
        try {
            ...
            while (sc.hasNextLine()) {
                ...
                template = String.valueOf(template) + currLine + "\r\n";
            }
            sc.close();
    ...
    private void checkURLAvailability(final String url) throws MalformedURLException, IOException {
    ...
    private void writeTagsForBranch(final String fileData, final String tagToIgnore) throws IOException {
        ...
        while (iter.hasNext()) {
            tags = String.valueOf(tags) + iter.next() + ",";
        }
        if (tags.endsWith(",")) {
    ...
    public void mergeABranch(final String aURL, final String branchName) {
        ...
        try {
            restURL = String.valueOf(restURL.endsWith("/") ? restURL : new StringBuilder(String.valueOf(restURL)).append("/").toString()) + "PRRestService/BuildSmoke/Application/Merge?UserIdentifier=BuildSmokeDeveloper&Password=cGVnYQ%3D%3D&branchlist=" + branchName;
