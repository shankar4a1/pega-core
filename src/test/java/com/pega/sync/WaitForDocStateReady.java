    ...
    private boolean waitForDocStateReady(final boolean debug) {
        ...
        while (stopWatch.getTime() < waitTimeout) {
            try {
                ...
                final List<WebElement> elements = (List<WebElement>)this.pegaDriver.findElements(By.xpath("//div[@class='document-statetracker']"));
                if (elements.size() > 0) {
                    everExisted = true;
                    ...
                    if (debug) {
                        WaitForDocStateReady.LOGGER.debug("DEBUG: Doc State element not found. (Don't panic!)", (Object)true);
                    }
                    this.sleep();
                }
            }
            catch (Exception e) {
                WaitForDocStateReady.LOGGER.error("Unexpected exception, will keep trying..." + e.toString(), (Object)true);
                this.sleep();
            }
        }
        if (!everExisted) {
